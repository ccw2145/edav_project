...
#cuisine_type <- df %>% select(cuisine) %>% group_by(cuisine) %>% summarize(count = n()) %>% #arrange(-count)
#ggplot(cuisine_type, aes(reorder(cuisine, count), count)) + geom_col() + coord_flip() +
#  labs(title = "Number of restaurants", x = "Cuisine")

#res <- cuisine_type[1:5, ]
...
violation_code_with_restype <- function(df, x){
...
dessert <- df %>% violation_code_restype("Dessert") %>% arrange(count)
...
p <- plot_ly(type = 'bar', orientation = 'h') %>%
  ...
  add_trace(x = dessert$count, y = dessert$violation_code, name = 'dessert', visible = F) %>%
