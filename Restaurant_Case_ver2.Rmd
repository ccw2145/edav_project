...
```{r Highest Violation Score Restaurant}
...
```{r STARBUCKS Most Freq Violation}
...
As Lisa show in the previous section, 10F and 08A are two most frequent violation in NYC. We decided to look at 08A violation particular in STARBUCKS since it represents the dimension we cared about more or more serious in food safety issue of the city - ‘potential presence of vermin (eg. mice).’
...
3.	A large portion of the STARBUCKS are located at Manhattan, comparing to where else. Those STARBUCKS did okay not the best in keeping restaurant tidy but not the worst either.
